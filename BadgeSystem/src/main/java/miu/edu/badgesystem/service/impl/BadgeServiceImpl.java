...
public class BadgeServiceImpl implements BadgeService {
    ...
    public BadgeResponseDTO findById(Long id) {
        Badge badge = badgeRepository.findById(id).orElseThrow(() -> new NoSuchElementException("Badge with id " + id + " NOT FOUND"));
    ...
    public List<BadgeResponseDTO> findAll() {
        ...
        return badges.stream().map(badge -> ModelMapperUtils.map(badge, BadgeResponseDTO.class)).collect(Collectors.toList());
    }
